...
describe "Test PJSNumberEditor", ->
	...
	it "check minValue validator with 1 object", ->
		...
		describe "Validation event", ->
			it "check validation-error event", (done) ->
				editor.on "validation-error", (_editor, _value, _errors) -> 
					expect(_editor).to.be.equal editor
					expect(_value).to.be.equal -31
					expect(_errors).to.be.length 1
					done()

				expect(editor.valueChanged(-31)).to.be.false

		# No maxValue
		expect(editor.valueChanged(9999999)).to.be.true
		expect(pjs.objectHandler.objs[0][settings.field]).to.be.equal 9999999

	it "check maxValue validator with 1 object", ->
