...
showObjects = ->	
	...
	$("#obj3 pre").html syntaxHighlight JSON.stringify obj3, null, 4
...
$ ->
	...
	showEditor = ->
		objs = []
		objs.push obj1 if $("#obj1 input:checked").length > 0
		objs.push obj2 if $("#obj2 input:checked").length > 0
		objs.push obj3 if $("#obj3 input:checked").length > 0
			
		pjs = new PJS ".propertyEditor", propertiesSchema, objs
		pjs.on "changed", (editor, value, objs) ->
			# alert "value changed to #{value}"

		pjs.on "function-clone", (editor, objs)->
			#alert "Clone selected #{objs.length} users"

		pjs.on "save", -> showObjects()

	
	showEditor()
	
	$(".objects input[type=checkbox]").on "change", -> showEditor()
